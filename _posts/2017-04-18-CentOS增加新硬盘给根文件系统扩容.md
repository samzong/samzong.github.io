...
然后启动 VM，这时可以通过<code>fdisk -l</code>查看当前已经增加了一块 12G 的磁盘

![](https://samzong.oss-cn-shenzhen.aliyuncs.com/blog/tat9r.jpg)

### 格式化磁盘并加入到 VGroup 组内

#### 查看当前<code>pvdisplay</code>磁盘列表，将<code>/dev/sdb</code>创建 pv 块
...
#### 查看当前<code>vgdisplay</code>VGroup 组，将<code>/dev/sdb</code>加入根目录所在 VGroup
...
#### 查看<code>lvdisplay</code>根卷的名称，将磁盘加入根卷的 LV
